...
class LiabilityController extends Controller
{
    public function list(){
        ...
        // dd($data['modal']); 
        $data['totalMoney'] = Order::where('total_money_paid', '!=', 0)->where('status', 1)->sum('total_money');
        $data['totalWeight'] = Order::where('total_money_paid', '!=', 0)->where('status', 1)->sum('total_weight');
        $data['totalMoneyPaid'] = Order::where('total_money_paid', '!=', 0)->where('status', 1)->sum('total_money_paid');
        return view('liabilities.customer_list',$data);
    }
    public function unpaidList($id){
        $data['order'] = Order::find($id);
        $data['order_list'] = Order::where('customer_id',$id)->where('total_money_paid', '!=', 0)->where('status', 1)->get();
        // dd($data['order_list']);
        $data['orderDetails'] = OrderDetail::where('order_id',$id)->get();
        $data['sum_weight'] = Order::where('total_money_paid', '!=', 0)->where('status', 1)->where('customer_id',$id)->sum('total_weight');
        $data['sum_money'] = Order::where('total_money_paid', '!=', 0)->where('status', 1)->where('customer_id',$id)->sum('total_money');
        $data['sum_paid'] = Order::where('total_money_paid', '!=', 0)->where('status', 1)->where('customer_id',$id)->sum('total_money_paid');
